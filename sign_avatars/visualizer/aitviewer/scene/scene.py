...
class Scene(Node):
    ...
    def light_mode(self, mode):
        if mode == "default":
            ...
            # self.ambient_strength = 2.0
            self.ambient_strength = 1.38
            self.floor.material.ambient = 0.65
            for l in self.lights:
                l.strength = 1.0
            for i in self.nodes:
                if i.name == 'SMPLSequence':
                    # i.mesh_seq.material.color = (1, 1, 1, 1)
                    i.mesh_seq.material.color = (0.8, 0.8, 0.9, 1.0)
            ...
            for i in self.nodes:
                if i.name == 'SMPLSequence':
                    i.mesh_seq.material.color = (0.8, 0.72, 0.425, 1)
