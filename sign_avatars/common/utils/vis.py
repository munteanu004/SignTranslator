...
try:
    import sign_avatars.common as _sa_common
    sys.modules['common'] = _sa_common
    sys.modules['common.utils'] = importlib.import_module('sign_avatars.common.utils')
    sys.modules['common.visualizer'] = importlib.import_module('sign_avatars.visualizer')
except Exception:
    pass
...
# Am eliminat complet logica PyTorch Patch.

os.environ['DISPLAY'] = ':0.0'
os.environ["PYOPENGL_PLATFORM"] = "egl"
torch.manual_seed(3407)

# Folosim CPU, deoarece placa video nu suportă CUDA
device = torch.device('cpu') 
print(f'Using device: {device}')

print('initialization...')
text_dict = {}
with open(os.path.join(_pkg_dir, 'datasets', 'hamnosys2motion', 'data.json'), 'rb') as f:
...
# A fost eliminat apelul .cuda() pentru a rula pe CPU
smplx_layer = copy.deepcopy(smpl_x.layer['neutral']) 
...
def get_coord(root_pose, body_pose, lhand_pose, rhand_pose, jaw_pose, shape, expr, cam_trans, mode='test', zero_global=False, mesh=False):
    ...
    # Mutăm zero_pose pe același device (CPU)
    zero_pose = torch.zeros((1, 3)).float().to(device).repeat(batch_size, 1)  # eye poses
    if not zero_global:
        output = smplx_layer(betas=shape.to(device), body_pose=body_pose.to(device), global_orient=root_pose.to(device), right_hand_pose=rhand_pose.to(device),
                             left_hand_pose=lhand_pose.to(device), jaw_pose=jaw_pose.to(device), leye_pose=zero_pose,
                             reye_pose=zero_pose, expression=expr.to(device))
    else:
        raise ValueError
    ...
    # Toate operațiile cu tensorii ar trebui să fie pe același device (CPU)
    x = (joint_cam[:, :, 0] + cam_trans.to(device)[:, None, 0]) / (joint_cam[:, :, 2] + cam_trans.to(device)[:, None, 2] + 1e-4) * \
        focal[0] + princpt[0]
    y = (joint_cam[:, :, 1] + cam_trans.to(device)[:, None, 1]) / (joint_cam[:, :, 2] + cam_trans.to(device)[:, None, 2] + 1e-4) * \
        focal[1] + princpt[1]
    ...
    # mesh
    if mesh:
        tx = (mesh_cam[:, :, 0] + cam_trans.to(device)[:, None, 0]) / (mesh_cam[:, :, 2] + cam_trans.to(device)[:, None, 2] + 1e-4) * \
            focal[0] + princpt[0]
        ty = (mesh_cam[:, :, 1] + cam_trans.to(device)[:, None, 1]) / (mesh_cam[:, :, 2] + cam_trans.to(device)[:, None, 2] + 1e-4) * \
            focal[1] + princpt[1]
        ...
        render_mesh_cam = mesh_cam + cam_trans.to(device)[:, None, :]  # for rendering
...
def process_single_video(file_name, result_path):
    ...
    # SOLUȚIA: Încărcare forțată cu pickle.load() pentru a evita PyTorch
    try:
        with open(file_name, 'rb') as f:
            results_dict = pickle.load(f)
            print("Successfully loaded file using standard pickle.load()")
            
    except Exception as e:
        # Mesajul de eroare din log ar trebui să fie afișat de patch-ul din serialization.py
        raise RuntimeError(f"FATAL: Nu s-a putut încărca fișierul {file_name}. Eroare: {e}")

    # Verificăm dacă rezultatul are cheile așteptate
    if 'focal' not in results_dict or 'smplx' not in results_dict:
        warnings.warn("Fișierul PKL încărcat nu conține cheile esențiale ('focal', 'smplx'). Voi folosi valori implicite.", UserWarning)
        focals = pred_focals
        princpts = pred_princpts
        all_pose = [] # Va eșua mai târziu, dar previne eșecul KeyError aici
    else:
        focals = results_dict.get('focal', pred_focals)
        princpts = results_dict.get('princpt', pred_princpts)
        all_pose = results_dict['smplx']

    # --- PATCH NOU: GESTIUNEA CHEILOR LIPSĂ (height/width) ---
    # Folosește .get() pentru a prelua valorile din PKL, sau folosește valorile predefinite dacă lipsesc.
    height = results_dict.get('height', predefined_height)
    width = results_dict.get('width', predefined_width)
    # --- END PATCH NOU ---

    # Asigurarea că datele sunt convertite în Tensor și mutate pe CPU
    try:
        all_pose = torch.tensor(all_pose, dtype=torch.float32).to(device)
    except Exception as e:
        raise RuntimeError(f"Eroare la conversia datelor 'smplx' în tensor: {e}. Datele ar putea fi corupte sau goale.")

    # Verificare pentru a evita erori la slicing dacă all_pose e gol
    if all_pose.numel() == 0:
        raise RuntimeError(f"Eroare: Datele 'smplx' din {file_name} sunt goale sau nu au putut fi încărcate corect.")
    ...
    if all_pose.shape[1] != 182:
        warnings.warn(f"Atenție: Dimensiunea datelor din 'smplx' este {all_pose.shape[1]}, dar se aștepta 182. S-ar putea să existe erori în vizualizare.", UserWarning)

    cam_trans_cpu = cam_trans.cpu() 
    
    # get_coord returnează un tensor pe device, îl mutăm pe CPU pentru numpy
    meshes = get_coord(g, b, l, r, j, s, exp, cam_trans_cpu[0][None], mesh=True).cpu().numpy()
    ...
    # Gestiunea numelui fișierului pentru text
    video_name_cleaned = video_name.lstrip('_-') 
    # Încercăm să extragem ID-ul video curat (partea de dinaintea primului underscore sau a primului număr de segment)
    match = re.search(r'([a-zA-Z0-9_-]+)_\d+-\d+', video_name_cleaned)
    if match:
        video_id_for_text = match.group(1).replace('/', '')
    else:
        video_id_for_text = video_name_cleaned.replace('/', '')

    text = text_dict.get(video_id_for_text, "Text Not Found")
    ...
    for idx, index in bar:
        ...
        if args.overlay:
            ...
            # Folosește focals/princpts salvate sau implicite
            img = render(raw_img.copy(), meshes[idx], smpl_x.face, {'focal': focals, 'princpt': princpts}) 
            img = np.array(np.concatenate((raw_img,img), axis=1), dtype=np.uint8)
            size = (2*width,height)
        else:
            # render with background
            # Folosește focalii predefiniți pentru fundalul static
