...
# Prefer the standard smplx package API, but fall back to the vendored
# layout in sign_avatars/common/utils/smplx when top-level import resolves
# to a namespace package without `create`.
try:
    import smplx as _smplx
    if hasattr(_smplx, "create"):
        smplx = _smplx
    else:
        from smplx import smplx as smplx
except Exception:
    from smplx import smplx as smplx

human_model_path = osp.join(osp.dirname(__file__), 'human_model_files')
class SMPLX(object):
    def __init__(self):
