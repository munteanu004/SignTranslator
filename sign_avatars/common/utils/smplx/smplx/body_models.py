...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        vertices, joints = lbs(shape_components, full_pose, self.v_template, # [1, 55, 3]
                               shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights, pose2rot=pose2rot,
                               )
        # print('1', joints.shape) # [1, 55, 3]
        ...
        landmarks = vertices2landmarks(vertices, self.faces_tensor,  # (68,3)
                                       lmk_faces_idx,
                                       lmk_bary_coords)

        # Add any extra joints that might be needed
        joints = self.vertex_joint_selector(vertices, joints) # [1, 76, 3]
        # print('2', joints.shape)
        # Add the landmarks to the joints
        joints = torch.cat([joints, landmarks], dim=1) # [1, 144, 3]
        # print('3', joints.shape)
        # Map the joints to the current dataset

        if self.joint_mapper is not None:
            joints = self.joint_mapper(joints=joints, vertices=vertices) # [1, 144, 3]
        # print('4', joints.shape)
        # exit()
